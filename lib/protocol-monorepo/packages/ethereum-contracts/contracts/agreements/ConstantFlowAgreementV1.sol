...
import {IConstantFlowAgreementV1, ISuperfluidToken} from "../interfaces/agreements/IConstantFlowAgreementV1.sol";
import {ISuperfluid, ISuperfluidGovernance, ISuperApp, FlowOperatorDefinitions, SuperAppDefinitions, ContextDefinitions, SuperfluidGovernanceConfigs} from "../interfaces/superfluid/ISuperfluid.sol";
...
contract ConstantFlowAgreementV1 is AgreementBase, IConstantFlowAgreementV1 {
    ...
    function updateFlowOperatorPermissions(
        ...
        newCtx = ctx;
        require(
            FlowOperatorDefinitions.isPermissionsClean(permissions),
        ...
        require(
            currentContext.msgSender != flowOperator,
            "CFA: E_NO_SENDER_FLOW_OPERATOR"
        );
    ...
    function _changeFlowToApp(
        ...
        // and its range will be [0, currentContext.appAllowance]
        {
            ...
            if (vars.appContext.appAllowanceUsed > 0) {
                ...
                vars.appContext.appAllowanceUsed = _clipDepositNumber(
                    vars.appContext.appAllowanceUsed.toUint256()
                ).toInt256();
            }
            ...
            // update flow data and account state with the allowance delta
            {
                vars.newFlowData.deposit = (vars
                    .newFlowData
                    .deposit
                    .toInt256() + appAllowanceDelta).toUint256();
            ...
            if (
                address(currentContext.appAllowanceToken) == address(0) ||
        ...
        )
    {
        uint256 newDeposit;
        {
            ...
            // STEP 1: calculate deposit required for the flow
            {
                ...
                appAllowanceBase = _calculateDeposit(
                    flowParams.flowRate,
                    liquidationPeriod
                );
            }
            ...
            if (
                address(appAllowanceToken) != address(0) &&
